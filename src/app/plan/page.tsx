  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-md flex-col gap-6 px-5 py-8">
          ...
          <div className="grid grid-cols-1 gap-2">
            {MODES.map((m) => (
              <button
                ...
                className={`p-3 text-left transition-transform active:scale-[0.98] ${
                  mode === m.id
                    ? "brutal-primary bg-primary text-primary-foreground"
                    : "glass"
                }`}
              >
                <span className="block font-black uppercase tracking-wide">{m.label}</span>
                <span className={`text-xs ${mode === m.id ? "text-primary-foreground/80" : "text-muted-foreground"}`}>
                  {m.hint}
                </span>
              </button>
            ))}
      ...
      {step === 2 && (
        <section className="flex flex-col gap-3">
          <p className="text-xs text-muted-foreground">
            Pick what you still need to do, then guess each duration from your
            gut <b>before</b> seeing what it typically takes. That guess is the
            rep — this is the gym.
          </p>
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 text-sm font-bold ${
                    sel ? "brutal-primary bg-primary text-primary-foreground" : "glass"
                  }`}
                >
                  {t.label}
                </button>
          ...
          {selections.map((s) => {
            ...
            return (
              <div key={s.taskId} className="brutal bg-card p-4">
                <p className="font-black uppercase tracking-wide">{s.label}</p>
                {s.planned === undefined ? (
                  <>
                    ...
                    {!s.revealed ? (
                      <Button
                        ...
                        onClick={() => reveal(s.taskId)}
                      >
                        Lock my guess & compare
                      </Button>
                    ) : (
                      <div className="mt-3 flex flex-col gap-2 text-sm">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <Button variant="secondary" size="sm" onClick={() => choose(s.taskId, Number(s.guess), "guess")}>
                            Keep {s.guess}m
                          </Button>
                          <Button variant="secondary" size="sm" onClick={() => choose(s.taskId, prior.p75, "prior")}>
                            Safe {prior.p75}m
                          </Button>
                          {med !== null && (
                            <Button size="sm" className="col-span-2 font-bold" onClick={() => choose(s.taskId, med, "history")}>
                              Trust my data: {med}m
                            </Button>
                          )}
                        </div>
      ...
      {step === 3 && timeline && now && (
        <section className="flex flex-col gap-3">
          ...
          {timeline.startAt.getTime() < now.getTime() && (
            <div className="brutal-alert bg-card p-3 text-sm font-bold text-destructive">
              This timeline starts in the past — you&apos;re already behind.
              Cut a task or accept the risk.
            </div>
          )}
          <ol className="flex flex-col gap-2">
            {timeline.steps.map((s) => (
              <li key={s.id} className="glass flex items-center justify-between p-3 text-sm">
      ...
      <div className="mt-auto flex gap-2 pt-4">
        ...
        {step < 3 ? (
          ...
          <Button
            className="brutal-primary flex-1 bg-primary font-black uppercase text-primary-foreground hover:bg-primary/90"
            disabled={!timeline}
            onClick={lock}
          >
            Lock timeline
